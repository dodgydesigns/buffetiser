...
from core.services.history import get_credit_debit_history


class Command(BaseCommand):
    """
    python manage.py bulk_import "/Users/mullsy/Downloads/statement.csv"
    """
    help = "This function takes a file containing CSV data and creates Purchases and Sales" \
        "objects from each row. Usage: " \
        "python manage.py bulk_import 'path_to_cmc_file'"
    ...
    def handle(self, *args, **options):
        ...
        #     if p := get_purchase_history(investment):
        #         print(f"BUY : {p}")
        #     if s := get_sale_history(investment):
        #         print(f"SELL: {s}")
            # print("-"*60)
        ...
        with open(options["file_path"]) as file_obj:
            ...
            for row in reader_obj:
                ...
                if len(description_list) > 3: # catch any blank rows
                    units, stock_code, price_per_unit = description_list[0], description_list[1], description_list[3]
                    investment, created = Investment.objects.get_or_create(key=f"XASX-{stock_code}")
                    if created:
                        investment.symbol = stock_code
                        investment.name = row[3]
                    if row[1] == "CB":
                        purchase = Purchase(investment=investment,
                                            units=units,
                        ...
                        sale = Sale(investment=investment,
                                    units=units,
                                    price_per_unit=price_per_unit,
                                    fee=11,
                                    date=datetime.strptime(row[0], "%d/%m/%Y"),
